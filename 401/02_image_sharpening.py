#image sharpening
#Amacımız daha keskin hatlara sahip görüntü elde etmektir.
import cv2 as cv
import numpy as np

src = cv.imread("/Users/bahriyeisgor/Desktop/OpenCV/images/foto.jpg")

sharpen_op = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]], dtype=np.float32)

sharpen_image = cv.filter2D(src, cv.CV_32F, sharpen_op)

sharpen_image = cv.convertScaleAbs(sharpen_image)
#convertScaleAbs : ölçekleme, mutlak değer alma, işaretsiz 8 bitlik bir türe dönüştürme işlemleri yapar.

cv.imshow("Sharpen Image", sharpen_image)
cv.waitKey(2000)